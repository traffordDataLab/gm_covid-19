...
rolling_cases <- cases %>% 
  ...
  mutate(ma_cases = rollmean(new_cases, 14, align = "left", fill = NA)) 

    div(class = "subtitle",
        div(class = "title", "Daily confirmed cases in Trafford"),
        paste("Pillar 1 testing, to", format(max(rolling_cases$date), '%d %B %Y')))
...
total_cases <- cases %>% 
  ...
  mutate(cum_cases = cumsum(new_cases))
    ...
    div(class = "subtitle",
        div(class = "title", "Cumulative cases in Trafford"),
        paste("Pillar 1 testing, to", format(max(total_cases$date), '%d %B %Y')))
...
rolling_rate <- cases %>% 
  ...
  mutate(rate = round(new_cases/population*100000,1), 
    ...
    div(class = "subtitle",
        ...
        paste("Pillar 1 testing, 14-day moving average, to", format(max(rolling_rate$date), '%d %B %Y')))
...
div(class = "graphic-div",
    div(class = "subtitle",
        ...
        paste("Pillar 1 testing, to", format(max(cases$date), '%d %B %Y'))
        ),
    tbl,
    div(class = "caption", p("Source: ", a("Public Health England", href = 'https://coronavirus.data.gov.uk/', target = "_blank")))
    )
