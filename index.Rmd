...
library(tidyverse) ; library(lubridate) ; library(reactable) ; library(zoo) ; library(ggtext) ; library(scales) ; library(shiny) ; library(ggiraph)
...
div(class = "graphic-div",
    div(class = "subtitle",
        div(class = "title", "Total confirmed cases in the last 14 days"),
...
rolling_rate <- cases %>% 
  ...
  group_by(area_name) %>% 
  mutate(label = str_c("<b style = 'color:#333333;font-size:12pt;'>", area_name, "</b><br/><span style = 'color:#757575;'>", comma(sum(new_cases)), " cases</span>"),
  ...
  mutate(tooltip =  paste0("<strong>", round(ma_rate,1), "</strong> cases per 100,000<br/>", area_name, "<br/>", date))
    ...
    gg <- ggplot(data = rolling_rate, aes(x = date, y = ma_rate, group = 1)) +
      geom_hline(yintercept = 0, size = 0.3, colour = "#333333") +
      geom_line_interactive(aes(colour = ifelse(date >= max(date)-14, "#39809E", "#bdbdbd"), tooltip = tooltip), 
                size = 1.2, show.legend = FALSE) +
      scale_color_identity() +
      scale_x_date(date_labels = "%b") +
      scale_y_continuous(limits = c(0, 10), expand = c(0.005, 0.005), position = "right") +
      labs(x = NULL, y = NULL, title = NULL, subtitle = NULL, caption = NULL) +
      theme_minimal(base_size = 16, base_family = "Lato") +
      theme(plot.margin = margin(0.5, 0.5, 0, 0.5, "cm"),
