...
<div class = "row">
<div class = "col-xs-12 col-sm-6 col-md-6 col-lg-6 graphic-div">
```{r dev=c('svg')}
...
rolling_rate <- cases %>% 
  ...
  group_by(area_name) %>% 
  mutate(label = str_c("<b style = 'color:#333333;font-size:14pt;'>", area_name, "</b><br/><span style = 'color:#757575;'>", comma(sum(new_cases)), "</span>"),
         cum_cases = sum(new_cases)) %>% 
  ungroup() %>% 
  mutate(period = case_when(date >= max(date)-14 ~ "last 14 days", TRUE ~ ""),
         label = fct_reorder(label, cum_cases, .desc = TRUE))
    ...
    ggplot(data = rolling_rate, aes(x = date, y = ma_rate, group = 1, colour = period)) +
      geom_hline(yintercept = 0, size = 0.3, colour = "#333333") +
      geom_line(size = 1, show.legend = FALSE) +
      scale_colour_manual(values = c("#bdbdbd", "last 14 days" = "#39809E")) +
