...
library(tidyverse) ; library(lubridate) ; library(reactable) ; library(zoo) ; library(ggtext) ; library(scales) ; library(shiny) ; library(ggrepel)
...
rolling_rate <- cases %>% 
  ...
  group_by(area_name) %>% 
  mutate(label = str_c("<b style = 'color:#333333;font-size:14pt;'>", area_name, "</b><br/><span style = 'color:#757575;'>", comma(sum(new_cases)), " cases</span>"),
  ...
  filter(area_name %in% c("Trafford", "Greater Manchester", "England"))
    ...
    ggplot(data = rolling_rate, aes(x = date, y = ma_rate, group = 1)) +
      ...
      geom_text_repel(aes(label = paste0(" ", format(date, "%d %B"))), data = . %>% group_by(area_name) %>% top_n(1, ma_rate), colour = "#212121", size = 4, nudge_y = 0.2) +
